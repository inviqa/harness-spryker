...
setup_app_networking()
{
    ...
    if ! grep "$YVES_HOST" /etc/hosts > /dev/null ; then
        DOCKER_INTERNAL_IP=$(/sbin/ip route|awk '/default/ { print $3 }')
        echo -e "$DOCKER_INTERNAL_IP    $YVES_HOST" | tee -a /etc/hosts > /dev/null
    fi

    if ! grep "$ZED_HOST" /etc/hosts > /dev/null ; then
        DOCKER_INTERNAL_IP=$(/sbin/ip route|awk '/default/ { print $3 }')
        echo -e "$DOCKER_INTERNAL_IP    $ZED_HOST" | tee -a /etc/hosts > /dev/null
    fi
